 ...
 **
 **Esta parte do módulo irá receber a informação por parte do cliente
 **e enviar as informações respetivas para o servidor, aguardando resposta do mesmo para informar
 **o cliente do estado do seu pedido. Este módulo receberá as informações do cliente 
 **acerca de uma passagem numa portagem e enviará para um servidor o mesmo pedido,
 **aguardando pelo mesmo e sendo atualizado em relação ao estado do pedido. Decorrido algum tempo,
 **quando souber se finalizou o pedido ou se deu erro, terminará o seu processo,
 **dizendo ao cliente se correu tudo bem ou se será necessário introduzir novamente 
 **a passagem, quer por erro de introdução do cliente, quer por erro de processamento do servidor.
 **
 **
 ******************************************************************************/
#include "common.h"
...
int getPidServidor() {
    ...
    servidor = fopen(FILE_SERVIDOR, "r");
     if ( servidor == NULL ){
        ...
        return -1;
     }else if (my_fgets(str, 20, servidor) == NULL){
        error("C1", "O ficheiro %s está em branco", FILE_SERVIDOR);
        return -1;
    }else{
        my_fgets(str, 20, servidor);
        pidServidor = atoi(str);
        if ( pidServidor > 0){
        success("C1", "O pid do Servidor é %d", pidServidor);
        }else{
        error("C1","O pid do Servidor não existe");
        return -1;
        }
        fclose(servidor);
    }
...
int escrevePedido(Passagem dados) {
    ...
    struct stat st; 
    int pid = getpid();
    int returnValue = access(FILE_PEDIDOS, F_OK);
